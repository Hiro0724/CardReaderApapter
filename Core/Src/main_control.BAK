...
#define CR_RES_LEN     (1)
#define CR_RES_ERR     (1)
#define CR_RES_ERRDECI (2)

#define CR_RES_ERRTM   (500/5)
				...
				if (p_Rxmail)
				{
					...
					switch(p_Rxmail->status)
					{
						...
						case RX_CRSERIAL:		// from card reader
 						...
 						case RX_CRSERIALRESPONSE:					// from card reader response timeout
							switch (mainstatus)
							{
								case STATUS_INIT:		            // 初期動作中
...
/*****************************************************************************
 *
 *	関数名称：void CR_SerialResponseCheak( void )
 *	処理概要：CRｼﾘｱﾙﾚｽﾎﾟﾝｽ判定処理　（5ms周期処理）
 *	引数	：なし
 *	戻り値	：なし
 *
 *****************************************************************************/
...
const	struct	jobtb tb_INIT[] =
{
    ...
    {QAPEV_CRRESTM, STATUS_INIT,tsk008},		// カード応答タイムオーバ
    {QAPEV_CRRESTM_DECI, STATUS_INIT,tsk009},	// カード応答タイムオーバ確定
